...
| スルーホールタップ（長）<br>TH-0.8-2.0-M2 | 16 | [マックエイト TH-0.8-2.0-M2](https://www.mac8sdk.co.jp/products/458) |
...
1. `スルーホールタップ（短）`を取り付ける。（左右それぞれ〓個、計〓個）

    ![〓取り付け位置](../assets/BuildGuide_DN0010/pcb_mid_inside_tap_short.png)

1. `スルーホールタップ（長）`を取り付ける。（左右それぞれ3個、計6個）
...
1. `スルーホールタップ（短）ネジなし`を取り付ける。（左右それぞれ〓個、計〓個）
...
1. `ボトムプレート`2枚と`ボトムカバー`1枚を`基板Base`に重ね、`樹脂ネジ（5mm）`でとめる。（左右それぞれ5箇所、計10箇所）
...
1. `基板Mid内側`に`スペーサー`を重ね、`ボトムカバー`から`金属ネジ（10mm）`でとめる。（左右それぞれ3箇所、計6箇所）
    ネジの頭部に、`テープ`を貼って絶縁する。
