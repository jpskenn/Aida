...
| スルーホールタップ（長）<br>TH-0.8-2.0-M2 | 16〓間違い？ | [マックエイト TH-0.8-2.0-M2](https://www.mac8sdk.co.jp/products/458) |
...
| 部品名 | 個数 | 備考 |
| :-- | :--: | :-- |
| Choc V2スイッチ | 40 | ピン折れや不良品などの予備に、5個くらい多めに用意しておくとよい。<br>Choc V1スイッチは非推奨だが、固定ピンがないもの、または固定ピンを取り外したものは取り付け可能と思われる。 |
| キーキャップ<br>スイッチに対応するもの | 必要数 | 一般的な英語104キーなどのキーキャップセットで良い具合に埋めることができる。（1.25uのAltやWindowsキーをスペースキーとして使用）<br>一部のMXスイッチ用のキーキャップは、組み合わせによってはスイッチプレートやスイッチに干渉する場合あり。<br>詳しくは[キーキャップの選び方](#キーキャップの選び方)を参照。 |
| USBケーブル （Type-C） | 1 | 基板Baseに取り付けられているコネクタはType-C。 |
| TRSケーブル | 1 | TRRSケーブルも使用できます。 |
...
1. 【該当者のみ】`LED`を取り付ける。（左右それぞれ3個。計6個）
    ...
    LED取り付け用パッド（1個のLEDに4個）のひとつに、予備はんだしておきます。  
    基板に描かれた印にLEDの端子の切り欠き部分を合わせ、LED発光部が下側（基板おもて側）になるように置きます。  
    ...
    〓確認方法未更新〓すべてのLEDを取り付けたら、USBケーブルでPCやMacに接続し、LEDがすべて白色に点灯することを確認します。  
...
1. `スルーホールタップ（長）`を取り付ける。（左右それぞれ〓個、計〓個）
